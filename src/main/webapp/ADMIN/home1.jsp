<link rel="stylesheet" href="../COMPONENTS/CSS/slider.css">
<jsp:include page="./COMPONENTS/nav.jsp" >
  <jsp:param name="button1Style" value="style='color:#19AA8D'" />
</jsp:include>   

<jsp:include page="../COMPONENTS/slider.jsp" >
  <jsp:param name="status" value="1" />
</jsp:include>   
   
<jsp:include page="../COMPONENTS/searchStream.jsp" >
  <jsp:param name="pageURL" value="../ADMIN/home1" />
  <jsp:param name="style" value="#F04F01" />
  
  <jsp:param name="icon1" value="&#xE417;" />
  
  <jsp:param name="icon2" value="file_download" />
  <jsp:param name="icon2url" value="../download" />
   
  <jsp:param name="icon3" value="&#xE254;" />
  <jsp:param name="icon3url" value="../ADMIN/home7" />
  
  <jsp:param name="icon4" value="&#xE872;" />
  
  <jsp:param name="icon5" value="" />
  <jsp:param name="icon5Style" value="style='display:none;'" />
   
  <jsp:param name="status" value="1" />
  <jsp:param name="title" value="Books Details" />
</jsp:include>   
</body>
</html>
